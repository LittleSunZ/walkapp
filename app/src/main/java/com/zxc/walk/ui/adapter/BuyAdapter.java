...
public class BuyAdapter extends BaseQuickAdapter<Transaction, BaseViewHolder> {
    ...
    public BuyAdapter(Context context) {
        super(R.layout.adapter_buy_item);
    ...
    protected void convert(@NonNull BaseViewHolder helper, Transaction item) {
        ...
        helper.getView(R.id.tv_safe).setOnClickListener(new View.OnClickListener() {
            @Override
                ...
                AlertDialog dialog = new AlertDialog.Builder(context).setNegativeButton("否", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (userInfo.getUserid().equals(item.getUserid())) {
                            CustomToast.showToast(context, "不能出售给自己!!", Toast.LENGTH_SHORT);
                            return;
                        }
                        if (onDealClickListener != null) {
                            onDealClickListener.onSafe(item);
                        }
                    }
                }).create();
                dialog.setTitle("提示");
                dialog.setMessage("是否确定出售" + item.getNumber() + "个雨露，单价￥" + item.getPrice() + "，需要扣除一定比例手续费");
                dialog.show();
            }
        });
    }
    ...
    public interface OnDealClickListener {
        void onSafe(Transaction transaction);
    }
